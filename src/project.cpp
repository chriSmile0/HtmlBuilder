...
int CheckOpenFile(std::string f, std::string type) {
	...
	if(retour == -1) {
		std::cerr << "Error " << f << " not readable, use " << type; 
...
void checkDAndC(std::string& save_option, int argc, int optind,
				std::string& out_opt, char *argv[], char& flag_mf_s,
				char flag_cs_mf_sy) {
	if(flag_cs_mf_sy != '-') {
		if(argc-optind == 0) {
...
int main(int argc, char *argv[]) {
	...
	while((opt = getopt_long(argc, argv, "usym:f:l:cd", options, &index)) != -1) {
		...
		switch(opt) {
			...
			case 'l': checkL(save_option, argc, optind, out_opt, out_opt2, argv,
						flag_mf_s, flag_cs_mf_sy);
				break;
			case 'c': checkDAndC(save_option, argc, optind, out_opt, argv,
						flag_mf_s, flag_cs_mf_sy);
				break;
			case 'd': checkDAndC(save_option, argc, optind, out_opt, argv,
					flag_mf_s, flag_cs_mf_sy);
				break;
			case 's'://Construction
				flag_cs_mf_sy = 's';
	...
	if(flag_cs_mf_sy == 's') { 
		...
		if(((flag != 'd') && (flag != 's')) && 
			...
			if(flag_mf_s == 's') {
				if(flag == 'd') 
					fileModificationAttributeTags(out_opt, file, styletag_test,
													save_option);
				else 
					fileModificationAttributeTags(out_opt, file, modification,
													save_option);
			}
			else if(flag_mf_s == 'm') {
				if(flag == 'd') 
